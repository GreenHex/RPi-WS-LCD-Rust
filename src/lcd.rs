...
pub mod lcd {
    ...
    pub struct Lcd {
        ...
        image: [u8; (LCD_WIDTH * LCD_HEIGHT * LCD_COLOUR_DEPTH) as usize],
    }
    ...
    impl Lcd {
        pub fn new(cs_pin: UBYTE, dc_pin: UBYTE, rst_pin: UBYTE, bl_pin: UBYTE) -> Self {
            Self {
                ...
                max_buffer_size: 32,
                image: [0x00; (LCD_WIDTH * LCD_HEIGHT * LCD_COLOUR_DEPTH) as usize],
            }
        }
        ...
        fn set_adjustment(&self) -> (UBYTE, UBYTE) {
            if self.memory_access_control_value() & 0x20 != 0x20 {
                (
                    ((LCD_X_CORRECTION >> 8) & 0xFF) as UBYTE,
                    ((LCD_Y_CORRECTION >> 8) & 0xFF) as UBYTE,
                )
            } else {
                (
                    ((LCD_Y_CORRECTION >> 8) & 0xFF) as UBYTE,
        ...
        pub fn lcd_set_window(
            ...
            spi_write_data_array(&[c0h, c0l + x_adj, c1h, (c1l - 1) & 0xFF + x_adj]);

            spi_write_ubyte2(&CmdOrData::Cmd(PAGE_ADDRESS_SET));
            spi_write_data_array(&[p0h, p0l + y_adj, p1h, (p1l - 1) & 0xFF + y_adj]);
        ...
        pub fn img_clear(&mut self, colour: UWORD) -> &Self {
            for i in (0..(LCD_WIDTH * LCD_HEIGHT * LCD_COLOUR_DEPTH)).step_by(2) {
                self.image[(i) as usize] = ((colour >> 8) & 0xFF) as u8;
                self.image[(i + 1) as usize] = (colour & 0xFF) as u8;
            }
            self
        }
        ...
        pub fn img_draw_rect(
            ...
            for j in y..(y + h) {
                k = j * LCD_WIDTH;
                for i in x..(x + w) {
                    self.image[(k + (i * LCD_COLOUR_DEPTH)) as usize] =
                        ((colour >> 8) & 0xFF) as u8;
                    self.image[(k + (i * LCD_COLOUR_DEPTH) + 1) as usize] = (colour & 0xFF) as u8;
                }
            }
            self
        }

        pub fn img_draw_pixel(&mut self, x: UWORD, y: UWORD, colour: UWORD) -> &Self {
            (*self).image[(y + x * LCD_COLOUR_DEPTH) as usize] = ((colour >> 8) & 0xFF) as u8;
            (*self).image[(y + x * LCD_COLOUR_DEPTH + 1) as usize] = (colour & 0xFF) as u8;
        ...
        pub fn img_draw_image(
            ...
            let mut chunks = self
                .image
                .chunks(((x_end - x_start) * LCD_COLOUR_DEPTH) as usize);
        ...
        pub fn img_print_data(&self) {
            let mut chunks = self.image.chunks((LCD_WIDTH * LCD_COLOUR_DEPTH) as usize);

            loop {
                match chunks.next() {
                    Some(c) => {
                        for i in 0..(LCD_WIDTH * LCD_COLOUR_DEPTH) as usize {
                            print!("0x{:02X} ", c[i]);
                        }
                        println!("\n");
                    }
    ...
    pub const LCD_INIT_SEQ: [CmdOrData; 47] = [
        ...
        Data(0x55), // 0x03 or 0x05 or 0x06?
